...
class ModelCode extends CCodeModel
{
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			...
			array('tablePrefix, modelPath, baseClass, buildRelations, buildCustom', 'sticky'),
		));
	}
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
			...
			if($this->buildCustom)
				$this->files[]=new CCodeFile(
					Yii::getPathOfAlias($this->modelPath).'/custom/'.$className.'.php',
					$this->render($templatePath.'/custom.php', array('modelClass'=>$className))
				);
		}
	}

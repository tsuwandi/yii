...
class ContentBase extends ActiveRecord
{
	...
	{
		return 'tblpa_content';
	}

	public function rules()
	{
		return array(
			array('content_name, description, admin_id', 'required'),
			...
			array('content_name', 'length', 'max' => 60),
			array('file_extension', 'length', 'max' => 20),
			// The following rule is used by search().
			array('id, content_name, description, file_extension, updated_time, admin_id, is_inactive', 'safe', 'on' => 'search'),
		);
	}
